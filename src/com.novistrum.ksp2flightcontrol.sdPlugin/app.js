...
function addButton(context, name) {
    let type = checkType(name);
    // Check if a button with the same context already exists
    let existingButton = buttons.find(button => button.context === context);
...
function getActionGroupState(button) {
    let randomUser = generateRandomString(6);
...
async function updateSingleButtonState(context) {
    let button = buttons.find(b => b.context === context);
    if (!button) {
